...
<head>
    ...
    <title>Propose Initiative - YB Dato' Zunita Begum</title>
...
<body>
    ...
    <main class="main">
        <div class="form-container">
            <h1 class="form-title">Propose Community Initiative</h1>
            <p class="form-description">
                Suggest community projects or initiatives that would benefit your area. Share your ideas for positive change.
            </p>

            <form method="POST" action="{{ route('public.forms.initiative.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-input" placeholder="Give your initiative a clear, descriptive title" required>
                    @error('title') <div class="error-message">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Target Location</label>
                    <input type="text" name="location" class="form-input" placeholder="Where will this initiative take place?" required>
                    @error('location') <div class="error-message">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Description</label>
                    <textarea name="description" class="form-textarea" placeholder="Describe your initiative in detail - what it is, who it benefits, and how it will be implemented..." required></textarea>
                    @error('description') <div class="error-message">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Expected Benefits</label>
                    <textarea name="expected_benefits" class="form-textarea" placeholder="Describe the expected benefits and impact on the community..." required></textarea>
                    @error('expected_benefits') <div class="error-message">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Estimated Budget (RM)</label>
                    <input type="number" name="estimated_budget" class="form-input" placeholder="0.00" min="0" step="0.01" required>
                    <div class="form-help">Provide a rough estimate of the total budget needed</div>
                    @error('estimated_budget') <div class="error-message">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Timeline</label>
                    <input type="text" name="timeline" class="form-input" placeholder="e.g., 3 months, 6 months, ongoing" required>
                    @error('timeline') <div class="error-message">{{ $message }}</div> @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="file-upload">
                        ...
                        <p class="form-help">Include any supporting documents, photos, or proposals (Max 5MB each)</p>
                    </div>
